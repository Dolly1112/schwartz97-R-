...
resid.schwartz2f.fit <- function(object, data, ttm, type = c("filter", "filter.std", "real"))
{
  ...
  if(type == "filter"){
    ...
    dimnames(vt) <- dimnames(data)    ##dimnames(x):重新设置对象的名称
    return(vt)
...
##lapply和sapply需要两个参数，第一个是输入数据，第二个是函数              return(matrix(NA, ncol = 1, nrow = nrow(vt)))
##区别在返回值，lapply返回list，sapply返回vector                     else
##chol2invCholeski分解求逆 ？？？？？？？？？？？？？？？                 return(solve(t(chol(Ft[,,i]))) %*% vt[,i])
                                                                     },
...
mean.schwartz2f <- function(x, time = 1)
{
  ...
  .mean <- function(time, object){
    ...
    sigma.log <- vcov(x, time = time)           ##Vcov() 列出模型参数的协方差矩阵
...
show.schwartz2f <- function(object)
{
    ...
    invisible(apply(cbind(names(tmp.coef), tmp.coef.formatted), 1,                 ##invisible:使变量不显示
                    function(x, max.len)cat(x[1], rep(" ", max.len - nchar(x[1])), ": ", x[2], "\n", sep = ""),
                    max.len = max(nchar(names(tmp.coef.formatted)))))              ##nchar的参数是一个字符向量，返回对应位置上字符串的长度
    cat("----------------------------------------------------------------\n")
}
### <---------------------------------------------------------------------->
...
plot.schwartz2f <- function(x, n = 100, time = 2, dt = 1/52)
{
  ...
  means <- mean(x, time = time.seq)     # Calculate expectations期望

  quantiles <- c(0.01, 0.05, 0.1, 0.9, 0.95, 0.99)      ##计算四分位值
