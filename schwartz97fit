...
fit.schwartz2f <- function(data, ttm, deltat = 1 / 260,
                           ...
                           rho = 0.7, lambda = 0,
                           meas.sd = rep(0.1, ncol(data)),                     ##ncol为矩阵的列
                           ...
                           r = 0.03, silent = FALSE, ...)
                                                     ...
                                                     ##rep(1:4, each = 2, times = 3, len = 12)
                                                     ##将1234每个重复2遍，一共循环3次，但总长度为12.长了会被截掉，短了会根据前面规则补上
                                                     ##所以为：[1] 1 1 2 2 3 3 4 4 1 1 2 2 

{
  ...
  log.likelihood.2f <- function(thetaOpt, thetaConst, thetaNames,
    ...
    {
      ## Transformations
      if(!is.na(thetaOpt["rho"])){                             ##na为缺失值
        thetaOpt["rho"] <- 2 * atan(thetaOpt["rho"]) / pi      ##atan()为arctan()
      }                                                        ##setdiff(x, y) [set difference]求向量x与向量y中不同的元素(只取x中不同的元素) 
      ...
      if(opt.meas.sd == "scalar"){                         ##标量scalar,只含有一个元素的向量
      ...
      logLikelihood <- fkf(a0 = stateSpace$a0,    ##$是S3类的引用方式。
                           P0 = stateSpace$P0,    ##当一个函数里需要返回多个值（比如有变量，有向量，有矩阵）时，我们要用list,而不是return，
                           Tt = stateSpace$Tt,    ##这时如果想提取某个变量的结果，就需要用到$                            
                           dt = stateSpace$dt,
                           HHt = stateSpace$HHt,
                           yt = stateSpace$yt,
                           Zt = stateSpace$Zt,
      ...
      if(!silent){
        cat("--> i: ", n.iter + 1,                                      ##cat连接
            ...
            "\n", sep = "")                             ##paste(1:5,c("st","nd","rd",rep(th,2)),sep"",collapse",")
      ...
      theta.backup <<- rbind(theta.backup, c(logLikelihood, rel.tol, abs.tol, thetaOpt))   ##rbind两个矩阵按行并列
